...
-- JERICHO module model:
--   * Deep mods (source-level patches of game internals — crumple, d2pl,
--     sandbox, antfarm, levelhacks) are COMPILED IN: premake auto-scans
--     JERICHO/MODS (a folder counts when it has a mod.toml WITHOUT
--     runtime = "dll") and links each into the game. MSVC cannot import
--     game data globals into a DLL without dllimport, so these cannot be
--     external plugins.
--   * API addons (mod.toml declares runtime = "dll") are built separately
--     by premake5_mods.lua into DLLs and loaded by the runtime loader
--     (jer_loader.c) — the game exe is never rebuilt for them.
-- No mod list is maintained anywhere: both pipelines auto-scan.
require "premake_modules/jericho_mods"

-- True when <id>'s mod.toml marks it as a runtime DLL addon.
function jericho_is_dll_addon(id)
...
-- Deep (compiled-in) modules: every installed mod that is NOT a DLL addon.
JERICHO_COMPILED_MODS = {}
for _, id in ipairs(jericho_scan_mods()) do
	if not jericho_is_dll_addon(id) then
		table.insert(JERICHO_COMPILED_MODS, id)
	end
end
...
-- JERICHO mod-host tooling: generates exports.def from the linker map so
-- mod DLLs can link against the game exe's symbols (see tools/gen_exports).
project "gen_exports"
	kind "ConsoleApp"
	language "c"
	files { "tools/gen_exports/**.c" }
...
project "REDRIVER2"
    ...
    filter "system:Windows"
        linkoptions {
			...
			"/MAP", -- linker map: gen_exports turns it into exports.def
			"/DEF:..\\exports.def", -- export every game symbol so mod DLLs can link
        }
    ...
    filter "system:linux"
        ...
        -- export every game symbol so dlopen'd mod .so files can link
        linkoptions {
            "-Wl,--export-dynamic",
        }
