...
| `JER_EVENT_PED_DRAW` | `JER_ARGS_PED_DRAW` | `newShowTanner` (`motion_c.c`) | ped body colour + per-instance palette: force a flat black (burning / bailed-out) or tinted ped, and/or select a recoloured outfit palette for this ped alone with `jer_ped_palette_select()` The flat colour reaches the polys as `plotContext.flatColour` under `PLOT_FLAT_COLOUR` (honoured in all three flat-shaded plot paths), **not** through `planeColours`: a pedestrian is drawn with `PLOT_NO_SHADE`, whose colour comes from `combo`, so holding `planeColours` alone - what the hook used to do - had no effect at all on a ped. Two things must also hold for it to cover the whole body: `RenderModel` preserves `PLOT_FLAT_COLOUR` across its `plotContext.flags = flags` assignment (otherwise a modelled head keeps `combo` — the "skin" stayed lit), and `DrawBodySprite` applies it to the limb quads, which never reach the plotter. |
