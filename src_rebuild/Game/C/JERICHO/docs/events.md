...
| `JER_EVENT_CAR_DATA_SOURCE` | `JER_ARGS_CAR_DATA_SOURCE` | `SetupResidentModels` (`mission.c`) | query: which city's `LEVELS\<city>` folder should this level read `CARMODEL_*` from, and which vehicles should be resident? `sourceLevel = 0..3` loads another city's vehicles (cross-city); `models[]` is the live `residentCarModels` array, so writing a model number into a slot (0..4 = traffic) puts that vehicle in the level. Runs before the model files are read |
