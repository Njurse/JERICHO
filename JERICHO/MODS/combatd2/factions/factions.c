...
// per-car assignment.
//
// (New concern, new sibling folder — see combatd2_internal.h for the file map.)
//
...
void cd2FacSetCar(int carId, int factionId)
{
	...
	if (gCd2Cfg.debugLog)
		printInfo("[combatd2] faction: car=%d -> %s (%s)\n",
...
int cd2FacPlayerFaction(void)
{
	...
	{
		if (gCd2Cfg.debugLog)
			printInfo("[combatd2] faction: player_faction %d is not playable, using TANNER\n", faction);
...
void cd2FacDumpField(void)
{
	...
	printInfo("[combatd2] factions: %d rows, player=%s, %d AI cars rostered\n",
	...
	for (i = 0; i < CD2_FAC_COUNT; i++)
	{
		...
		printInfo("[combatd2] faction %d %-8s %-15s %-17s rgb=%02X%02X%02X competes=%d rank=%d cars=%d/%d special=%s\n",
		...
		printInfo("[combatd2] faction %s wants: %s | cry: %s\n",
			f->tag, (f->motive != NULL) ? f->motive : "-", cry);

		if (expected != f->carsPerLevel)
			printInfo("[combatd2] factions: WARNING %s says carsPerLevel=%d but the roster fields %d\n",
...
void cd2FacRegister(JERICHO_CONTEXT* ctx)
{
	...
	ctx->jer_log(ctx, "[combatd2] factions registered (SDK v%d)\n", ctx->sdkVersion);
}
