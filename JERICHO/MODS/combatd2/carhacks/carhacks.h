 ...
 * its own JERICHO module later without touching combatd2:
 *   - it uses only engine globals (already exported to mods via exports.def)
 *     and the public JERICHO API - no combatd2 internals;
 ...
 *     "carhacks", so no combatd2 name is reachable from here.
 ...
 * carhacks_register(ctx) call out of combatd2's module entry, and add it to
