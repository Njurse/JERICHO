...
Live uses today: the kill banner's names (`cainescrossfirewreckfx.c`), the opponent
...
Config (`JERICHO/CONFIG/cainescrossfire.ini`), alongside `factions`/`player_faction`:
...
- **A different player team** — `cainescrossfire.ini`: `player_faction = <id>` (0
...
- **The player's own name in the banner** — `cd2cAnnounce` in
  `cainescrossfirewreckfx.c` returns `"You"` for the local player. That single return
...
`[cainescrossfire] debug_log = 1`:

```
[cainescrossfire] factions: 5 rows, player=TANNER, 4 AI cars rostered
[cainescrossfire] faction 0 TANNER   John Tanner     Rogue Undercover  rgb=E6EEF8 competes=1 rank=0 cars=1/1 special=MISSILE
[cainescrossfire] faction TANNER wants: Caine's criminal network exposed - ... | cry: You're coming down with me.
...
