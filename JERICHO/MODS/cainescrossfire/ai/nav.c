...
static int cd2NavIsqrt(long long v)
{
	long long r = 0;
		...
		if (lineClear(&leg[0], &leg[1]) == 0)
		{
			...
			if (cd2NavRepairLeg(&leg[0], &leg[1], &mid))
			{
				...
				if (gCd2Cfg.debugLog && (sRepairLog++ % 20) == 0)
					printInfo("[cainescrossfire] nav: repaired blocked leg (%d,%d)->(%d,%d) via (%d,%d)\n",
...
static void cd2NavBuild(void)
{
	...
	if (gCd2Cfg.debugLog)
		printInfo("[cainescrossfire] nav graph built: nodes=%d (S%d C%d J%d) edges=%d\n",
...
int cd2NavRoute(int carId, const VECTOR* from, const VECTOR* goal, CD2_NAV_ROUTE* out)
{
	...
	if (replan)
	{
		...
		if (gCd2Cfg.debugLog)
		{
			...
			if ((t++ & 31) == 0)
				printInfo("[cainescrossfire] nav route car=%d src=%s wp=%d len=%d straight=%d goalNode=%d expanded=%d\n",
					carId,
