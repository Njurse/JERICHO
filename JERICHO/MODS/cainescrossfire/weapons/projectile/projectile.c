			...
			int count, int interval)
{
	...
	if (gCd2Cfg.debugLog)
		printInfo("[cainescrossfire] volley start: %s x%d every %d frames (volley=%d)\n",
				...
				for (j = 0; j < MAX_CARS; j++)
				{
					...
					if (cd2WpnPointInCar(cp, &p->pos))
					{
						...
						if (v >= 0 && gVolley[v].active && gVolley[v].gen == vg)
						{
							...
							if (gCd2Cfg.debugLog)
								printInfo("[cainescrossfire] volley land %d/%d car=%d\n",
							...
							if (gVolley[v].landed >= gVolley[v].count)
							{
								...
								if (gCd2Cfg.debugLog)
									printInfo("[cainescrossfire] volley COMPLETE %d/%d bonus dmg=%d car=%d\n",
