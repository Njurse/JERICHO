# Caine's Crossfire opponent AI

The opponent cars you fight in Caine's Crossfire. The brain is `ai/opponent.c` (the large
file); `ai/nav.c`, `ai/grid.c` and `ai/flow.c` are the navigation layer it steers
with. Line references are into `JERICHO/MODS/cainescrossfire/` unless stated otherwise;
...
opponent.c:1587-1590) through the **same Caine's Crossfire handling path the player uses**,
...
Four archetypes, `CD2_AI_ROLE_*` (cainescrossfire.h:409-416), named by
...
The states are `CD2_AI_*` (cainescrossfire.h:395-404) and their display names come from
...
(`cd2CarBrake` reads the car's Caine's Crossfire stats, cainescrossfire.c:300-308). If
...
352, cainescrossfire.h:52; full throttle `CD2_TMB_THRUST` 4215, cainescrossfire.h:105). Without
...
overridden for a single headless run with the `CC_OPPONENTS` environment variable
